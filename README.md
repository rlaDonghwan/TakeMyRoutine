...
![2학년 2학기 시간표](https://github.com/rlaDonghwan/TakeMyRoutine/assets/115167094/d9e15cb3-afa1-4e42-8d74-0fd9c7a1125c)
...
## 첫 메인 화면
메인 로그인 화면에서 로그인 버튼을 클릭하거나 추가하기 버튼 또는 캘린더를 클릭하게 되면 바로 로그인 화면으로 넘어 갈 수 있게 설계하였다.
...
## 로그인 실패 화면
값을 입력하지 않거나 값이 일치하지 않으면 경고창이 띄게 된다.

## 회원 가입 화면
![회원 가입 화면](image.png)

## 회원 가입 실패 화면
닉네임이 중복되거나 비어 있는 경우 저런식으로 경고 문구가 뜨게 된다.

## 로그인 후 메인 화면
![로그인 후 메인 화면](image.png)

## 로그인 세션 처리 화면
로그인이 완료 되면 사용자에 닉네임이 상단에 유지 된다.

## 추가 버튼 클릭 시 화면
Todo를 입력할 수 있는 폼으로 넘어오게 된다.

## 수정 하기 버튼 클릭 시 화면
![수정 하기 버튼 클릭 시 화면](image.png)

## 완료 버튼 클릭 시 화면
경고창에서 확인을 누르게 되면 리스트에서 값이 사라지고 캘린더에서도 같이 사라지된다.

## 캘린더 날짜 클릭 시 추가 모달 창
캘린더에서 날짜 부분을 클릭하게 되면 추가하기 모달창이 뜨게 된다.

## 캘린더 이벤트 수정 및 삭제 모달 창
기존에 존재하는 이벤트를 클릭하게 되면 값을 수정하거나 삭제할 수 있는 모달 창이 띄게 된다.

## Todo 루틴화 버튼 클릭 시 화면
선택된 Todo가 2개 이상이 될 시 루틴화 버튼이 활성화 된다.
활성화된 버튼을 클릭 시 Todo 루틴화 모달창이 뜨게 된다.

## 루틴 페이지 화면
루틴화 한 본인의 목록과 공유된 리스트를 같이 볼 수 있다.

## 공유된 리스트
공유가 성공적으로 완료 되면 리스트에 공유된 리스트가 뜨게 된다.

## 공유된 리스트
공유된 리스트는 모든 사용자가 확인 할 수 있다.
위에 닉네임을 보면 다른 사용자가 로그인 했는데도 공유된 리스트를 볼 수 있다.

## 리스트에서 Take 버튼 클릭 시 화면
공유된 리스트에 Take 버튼을 클릭하면 루틴 정보 모달창이 뜨게 된다.

## 선택 후 Take버튼 클릭 시 화면
다운로드가 성공 되었다는 알림이 띄게 된다.

## Take가 완료된 다른 유저의 화면
다운로드가 완료 된후 메인 페이지로 넘어가게 되면 그 정보가 넘어오게 된다.
